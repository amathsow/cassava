...
class EgoCNN:
    ...
    #Arguments
        ...
        out_size: tensor shape of the output(s), should be coherent with tbins and xrange, yrange
        handle: hdf5 dataset handle for the batch_generator method
        xrange: (min,max) tuple with the extremes of x random translations (in pixels)
        yrange: (min,max) tuple with the extremes of y random translations (in pixels)
    ...
    def __init__(self, in_size, out_size,
        ...
        self.xrange = (xrange[0], xrange[1]+1) # +1 because np.random.randint samples from [min, max+1]
        self.yrange = (yrange[0], yrange[1]+1) # +1 because np.random.randint samples from [min, max+1]
        self.lscale = lscale
        self.trange = trange
        self.tbins = tbins
    ...
    def bcnn(self):
        ...
        x = Conv2D(96, kernel_size=(7,7), strides=(1,1), padding='same', activation='relu')(input_tensor)
    ...
    def tcnn(self, bottom_shape=None):
        '''
        Conv2D and Fully connected layers to be plugged on top of the concat siamese CNNs.
        #Arguments
    ...
    def batch_generator(self, handle):
        ...
        while True:
            ...
            # apply transformations to a batch (hdf5 dataset accepts lists for slicing, not np.arrays)
...
class EgoCNNReg(EgoCNN):
    ...
    def batch_generator(self, handle):
        ...
        while True:
            ...
            Xbatch = handle[list(indices)]
            yield ({'input0': Xbatch, 'input1': Xbatch_trans},
                   {'output0': z_rotat, 'output1': x_trans, 'output2': y_trans})
